...
function show_home() {
	if (currentTab != "home") {
		currentTab = "home";
		if (hasClass(holder_div, 'open') === true) {
			hideAll();
			setTimeout(callback(home_div.innerHTML), 700);
		} else {
			setTimeout(callback(home_div.innerHTML), 0);
		}
	}
}
...
function show_team() {
	if (currentTab != "team") {
		...
		if (hasClass(holder_div, 'open') === true) {
			hideAll();
			setTimeout(callback(team_div.innerHTML), 700);
		} else {
			setTimeout(callback(team_div.innerHTML), 0);
		}
	}
}
...
function show_games() {
	if (currentTab != "games") {
		...
		if (hasClass(holder_div, 'open') === true) {
			hideAll();
			setTimeout(callback(games_div.innerHTML), 700);
		} else {
			setTimeout(callback(games_div.innerHTML), 0);
		}
	}
}
...
function show_films() {
	if (currentTab != "films") {
		...
		if (hasClass(holder_div, 'open') === true) {
			hideAll();
			setTimeout(callback(films_div.innerHTML), 700);
		} else {
			setTimeout(callback(films_div.innerHTML), 0);
		}
	}
}
...
function show_contact() {
	if (currentTab != "contact") {
		...
		if (hasClass(holder_div, 'open') === true) {
			hideAll();
			setTimeout(callback(contact_div.innerHTML), 700);
		} else {
			setTimeout(callback(contact_div.innerHTML), 0);
		}
	}
}
...
function show(){
	holder_div.setAttribute("class", "open");
}

function hide() {
	holder_div.setAttribute("class", "close");
}

function callback(content){
    return function(){
    	holder_div.innerHTML=content;
		show();
		resize();
    }
}
